...
class MachineLearningJZ(Algorithm):
    def __init__(self):
        ...
        # Belohnungen
        self.reward_system.reward_eat_food = 0.1
        self.reward_system.reward_win = 0.05
        self.reward_system.reward_closer_function = lambda closer: 0.01 if closer > 0 else -0.005
        # Bestrafungen
        self.reward_system.reward_killed_by_wall = -0.05
    ...
    def train(self, spielfeld: GameData, aktion: str, reward: float) -> None:
        ...
        if reward < 0:
            ...
            saeule[nummer_der_aktion] += reward
            if saeule[nummer_der_aktion] < -1:
                saeule[nummer_der_aktion] = -1
        elif reward > 0:
